...
const Logo = ({ isOpen }: { isOpen: boolean }) => {
  return (
    <>
      <svg
        className={`h-[25px] w-[25px] shrink-0 cursor-pointer md:ml-[11px] md:h-[22px] md:w-[22px] md:duration-200 md:ease-in-out xl:ml-[12px] xl:h-[30px] xl:w-[30px]`}
        id="logo-85"
        width="40"
        height="40"
        viewBox="0 0 40 40"
      ...
      {/* Logo Text */}
      <h2
        className={`truncate md:overflow-hidden md:duration-300 ${isOpen ? "md:max-w-full" : "md:max-w-0"} relative w-fit bg-gradient-to-r from-[#5417D7] via-[#6d50c4] to-[#9b4dff] bg-clip-text text-center text-[28px] font-[700] leading-[28px] text-transparent md:text-[20px] xl:text-[32px] xl:leading-[40px]`}
      >
        TrackIt
      </h2>
    </>
...
const NavBar: React.FC<NavBarProps> = ({ isOpen }) => {
  ...
  const DashboardIcon: React.FC<NavBarProps> = ({ isOpen }) => (
    <svg
      className={`${isOpen ? "md:mx-[0px]" : "md:mx-[2px]"} h-[22px] w-[22px] shrink-0 md:h-[16px] md:w-[16px] md:duration-300 md:ease-in-out xl:h-[23px] xl:w-[23px]`}
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
  ...
  return (
    <>
      {/* Sidebar Navigation */}
      <nav
        className={`absolute left-0 top-[65px] grow md:relative md:left-[unset] md:top-[unset] ${isOpen ? "max-md:visible max-md:translate-x-0" : "max-md:invisible max-md:translate-x-[-100%]"} w-full bg-white pt-[20px] duration-300 ease-in-out md:pt-[30px]`}
      >
        <div className="relative mx-auto flex h-full w-[90%] flex-col md:mx-[10px] md:h-full md:w-[unset] lg:mx-[8px] xl:mx-[12px]">
            ...
            <ul>
              ...
              >
                ...
                >
                  ...
                  {/* User Name and Email */}
                  <div
                    className={`md:overflow-hidden md:duration-300 ${isOpen ? "md:max-w-full" : "md:max-w-0"} text-left text-[18px] leading-[28px] md:text-[14px] md:leading-[20px]`}
                  >
                    <span
                  ...
                  <button onClick={() => logout()} className="ml-auto">
                    <FiLogOut
                      className={
                        isOpen
                          ? "w-20px h-[20px] text-red-500 duration-500 xl:h-[22px] xl:w-[22px]"
                          : "h-0 w-0"
                      }
                    />
                  </button>
